...
{{--                                        <th scope="row"> <input type="checkbox" class="form-check-input checkbox-prod-detail" name="id[]" value="{{ $cart->id }}"></th>--}}
                                        <input type="hidden" class="form-check-input checkbox-prod-detail" name="id[]" value="{{ $cart->id }}">
                                            ...
                                            <div>
                                                ...
                                                @if(!empty($cart->productVariant))
                                                    <div class="header__top__right__language">
                                                        <div>Phân loại: <b> {{$cart->productVariant->variant_type}}</b>
                                                        </div>
                                                        ...
                                                        <ul style="color: #ffffff">
                                                            @foreach ($variantValues as $key => $item)
                                                                @if ($key == 'color')
                                                                    <li>
                                                                        <a>{{$key}}:
                                                                            <span style="background-color: {{ $item }}; border-radius: 4px; color:{{ $item }}; font-size: 8px">{{ $item }} </span>
                                                                        </a>
                                                                    </li>
                                        ...
                                        <td class="shoping__cart__quantity">
                                            <div class="quantity">
                                                <div class="pro-qty">
                                                    <input type="text" name="quantity[]" data-prod-id="{{ $cart->id }}"
...
@section('javascript')
    ...
    <script>
        ...
        proQty.on('click', '.qtybtn', function () {
            ...
            updateProductCarts(prodId,newVal)
